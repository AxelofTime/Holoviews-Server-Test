...
def produce_timehistory(doc, ipm2List, ipm3List, ebeamList, ipm2TS, ipm3TS, ebeamTS):
    ...
    b_th_ipm_std = Buffer(pd.DataFrame({'lowerbound':[], 'higherbound':[]}), length=1000000)
    ...
    test1 = datashade(plot_ipm_std_low, streams=[hv.streams.PlotSize], normalization='linear') 
    #.opts(norm=dict(framewise=True))
    test2 = datashade(plot_ipm_std_high, streams=[hv.streams.PlotSize], normalization='linear')
    #.opts(norm=dict(framewise=True))
